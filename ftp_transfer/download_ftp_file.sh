...
echo /bin/bash $script_folder"/download_file.sh" $host $username "PASSWD" $source $target
/bin/bash $script_folder"/download_file.sh" $host $username $password $source $target 1> ~/ftp.txt 2>&1

cat ~/ftp.txt | grep -v "Interactive mode off."
if [ $? -ne 0 ] ; then
    echo "*****************"
    echo "download  $source to $ftpfile successfully"
    echo "*****************"
    exit 0
else
    echo "*****************"
    echo "download  $source to $ftpfile failed"
    echo "*****************"
    exit 2020
fi
